...
- ✅ 里程碑 5（阶段4）已完成：高级文件操作与在线编辑
    ...
    - 新增 `archive_pack_stream_download`（tar.gz / tar / tar.bz2 / tar.xz / zip），支持勾选路径远端打包并流式下载到传输队列
    - `transfer_upload` 新增上传后自动解压，支持原地解压 / 新目录解压 / 自定义目录，并可选删除远端压缩包
    - 当前限制：仅支持 UTF-8 文本；压缩下载依赖远端 `tar/zip`；自动解压依赖远端 `tar/unzip`
- ✅ 里程碑 6（阶段1）已完成：左侧监控面板基础采集
    - `terminal_connect_ssh` 返回 `connection_id`，供监控订阅复用
    - 新增 `monitor_subscribe/monitor_unsubscribe/monitor_set_path_probe` 与 `monitor.update` 事件
    - 左侧面板已接入 CPU/内存/负载/网速、磁盘挂载、路径探测（basic/disk/path_probe）
    - 当前限制：仅支持直连 SSH 主机；`dir_size_bytes` 仍未采集
- ✅ 里程碑 6（阶段2）已完成：监控图表化与采样节奏优化
    - 新增 CPU / MEM / Rx / Tx 迷你曲线图，基于最近 30 次采样绘制
    - 磁盘挂载区升级为占用率条形图，展示已用/总量/可用空间
    - 前端切换活动 SSH 标签时，会主动释放非活动连接的监控订阅，降低采样开销
    - 当前限制：历史数据仍为内存态，未持久化
- ✅ 里程碑 6（阶段3）已完成：监控会话仪表板与 SSH 引导优化
    - 中央会话区新增 SSH 概览仪表板，展示连接摘要、核心指标与趋势条
    - 文件栏新增“在终端打开”，便于把当前远端目录快速切到终端上下文
    - 非 SSH / 无 Host 场景增加明显引导，降低“为什么没连上 SSH”的认知成本
    - 当前限制：`dir_size_bytes` 仍未采集，尚未实现独立的大图弹层
- 🚧 里程碑 7（阶段1）进行中：进程管理 + systemctl 服务管理
    - 新增 `System` 页面，按 Host 拉取远端进程列表与 systemctl 服务状态
    - 后端新增 `process_list_remote/process_signal_remote/service_list_remote/service_action_remote`
    - 当前动作支持 TERM/KILL 信号，以及 start/stop/restart/reload 服务控制
    - 当前限制：仅支持直连主机；systemctl 仅在 systemd 环境可用；暂不支持批量动作
- ▶ 下一执行目标：里程碑 7（阶段2：进程筛选增强、更多信号与服务详情）
...
5. 文件栏：表格列齐全（名/大小/类型/修改时间/权限/用户组），上传下载队列可靠；支持远端压缩流式下载、上传后解压；在线编辑小文件并保存前展示
   diff。
...
- 远端压缩下载（核心）
    - archive.pack_stream_download(connection_id, paths[], format: tar_gz|tar|tar_bz2|tar_xz|zip, local_path?) -> {
      transfer_id }
    - 后端选择命令优先级：tar 系列优先走 `tar`，zip 走 `zip`；缺依赖则报错
- 上传后解压
    - `transfer.upload` 扩展字段：`extract_after_upload`、`extract_destination`、`remove_archive_after_extract`
    - 前端目录策略固定枚举：same_dir | new_dir | custom，最终统一落为 `extract_destination`
    - 流程固定：上传完成后按策略选择解压目录，可选删除远端压缩包
